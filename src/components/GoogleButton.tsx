...
/** Reusable "Sign in / Sign up with Google" button. Google login is optional. */
export function GoogleButton({ onClick, label, disabled }: GoogleButtonProps) {
  return (
    <Button
      type="button"
      variant="outline"
      className="w-full h-12 text-base hover:bg-muted text-foreground border-input hover:text-foreground"
      onClick={onClick}
      disabled={disabled}
    >
